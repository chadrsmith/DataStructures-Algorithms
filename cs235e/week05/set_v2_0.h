...
private:
    ...
    void insertIndex();     // private function for inserting item at a specific index
    void reallocate();      // private function for reallocating the data variable
    ...
    if (foundIndex < 0) {
        ...
        for (int i = 0; i < numItems; i++) {
            ...
            if (t > data[i] && t < data[i+ 1]) {
                ...
                for (int x = (numItems - 1); x >= shiftStartPoint;  x--) {
                    data[x + 1] = data[x];
                }
...
/***************************************************
 * SET :: FIND
 * Searches for the passed arguement. If the data
 * exists then it will return it's index in the set
 * if it doesn't then it will return a -1
 **************************************************/
template <class T>
